...
test('recipe swipe deletion is coordinated, full-height, and transform-only', () => {
  ...
  assert.match(card, /onTouchStart=\{handleTouchStart\}/)
  assert.match(card, /onTouchMove=\{handleTouchMove\}/)
  assert.match(card, /onTouchEnd=\{handleTouchEnd\}/)
  assert.match(card, /translate3d\(\$\{offset\}px, 0, 0\)/)
  ...
  assert.match(styles, /\.recipe-card__swipe-track\{[^}]*transition:transform \.18s/)
  assert.match(styles, /\.recipe-card__swipe-action\{[^}]*align-self:stretch/)
