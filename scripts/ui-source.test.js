...
test('recipe swipe deletion is coordinated, full-height, and native-driven', () => {
  ...
  assert.match(styles, /\.recipe-card__swipe-area\{[^}]*position:absolute[^}]*height:100%/)
  assert.match(styles, /\.recipe-card__swipe-movable\{[^}]*height:100%/)
  assert.match(styles, /\.recipe-card__swipe-action\{[^}]*top:0[^}]*bottom:0/)
  assert.match(styles, /\.recipe-card__swipe-action\{[^}]*right:-2px[^}]*width:calc\(4\.5rem \+ 2px\)[^}]*overflow:hidden[^}]*border-radius:\$radius-lg/)
